...
def doArgs(argList, name):
    ...
    parser.add_argument('--command', action="store", dest="command", type=str, help="Minizinc base command", required=True)
...
def foundSolMinizinc(output):
    return not output[0].startswith("=====UNSATISFIABLE=====")

# execute a command with a timeout and the parameter par equal to n
def executeWithTimeoutMinizinc(command, timeout, n, par="maxT"):
    c = 'timeout -s 15 -k 5 ' + str(timeout) + " " + command + " -D " + par + "=" + str(n)
...
def getSolutionMinizinc(output):
    return output[0]
...
def findMaxPar(command, timeout,nuke_p, par="maxT"):
    ...
    while(True):
        try:
            ...
            output = executeWithTimeoutMinizinc(command, timeout, n, "maxT")
            if foundSolMinizinc(output):
                solution = getSolutionMinizinc(output)
                print(solution)
...
def main():
    ...
    out = findMaxPar(command, timeout,nuke_p)
